    ...
    <div className="relative w-64 h-[520px] bg-[#0a0c14] rounded-[2rem] border-4 border-[#1a1f28] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)] flex-shrink-0">
      ...
      <div className="px-4 py-3 h-full">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-1.5 text-[9px] font-mono uppercase tracking-widest text-[#39ff88]">
        ...
        {/* Buy signal */}
        <div className="bg-[#070712] rounded-sm p-3 mb-3 border border-[#39ff88]/30 relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-1 h-full bg-[#39ff88]" />
          <div className="flex justify-between items-center mb-1.5 pl-2">
            <span className="text-[10px] font-mono font-bold text-[#39ff88] tracking-widest uppercase">BUY SIGNAL</span>
          ...
          <div className="flex justify-between items-center mb-3 pl-2">
            ...
            <span className="text-[11px] font-mono text-[#39ff88] font-bold">+1.8% target</span>
          </div>
          <button className="w-full bg-[#39ff88]/10 hover:bg-[#39ff88]/20 text-[#39ff88] text-[10px] font-mono font-bold tracking-widest py-2 rounded-sm transition-colors border border-[#39ff88]/20 flex items-center justify-center gap-2">
            EXECUTE SCALP 
      ...
      <div className="px-4 py-3">
        ...
        <div className="space-y-2 mb-5">
          {trades.map(({ pair, entry, pnl, pct, up }) => (
            <div key={pair} className={`bg-[#070712] rounded-sm p-3 border-l-2 ${up ? "border-[#39ff88] bg-[#39ff88]/5" : "border-[#ff4d6d] bg-[#ff4d6d]/5"}`}>
              <div className="flex justify-between items-center">
                <span className="text-[13px] font-bold text-white tracking-wide">{pair}</span>
                <span className={`text-[13px] font-mono font-bold ${up ? "text-[#39ff88]" : "text-[#ff4d6d]"}`}>{pnl}</span>
              </div>
              <div className="flex justify-between mt-1.5 text-[10px] font-mono text-[#5f6470]">
                <span>ENTRY: {entry}</span>
                <span className={up ? "text-[#39ff88]" : "text-[#ff4d6d]"}>{pct}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-[#070712] rounded-sm p-4 border border-[#1a1f28]">
          ...
          <div className="flex justify-between items-end">
            <span className="text-2xl font-mono font-black text-[#39ff88] tracking-tight">+$876.42</span>
            <div className="text-right">
              <span className="block text-[9px] font-mono text-[#5f6470] uppercase tracking-wider mb-0.5">12 trades</span>
              <span className="text-[9px] font-mono text-[#39ff88] bg-[#39ff88]/10 px-1 py-0.5 rounded-sm">83% WIN</span>
            </div>
          </div>
        </div>
      </div>
