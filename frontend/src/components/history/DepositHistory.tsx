...
function truncateAddress(address: string, start = 7, end = 4) {
  if (!address) return "";
...
export default function DepositHistory({ transactions }: HistoryProps) {
  ...
  if (transactions.length === 0) {
    return <p className="text-xs text-gray-500 py-10 text-center">No recent deposits found.</p>;
  }
  ...
  return (
    <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1 scrollbar-none">
      {transactions.map((tx) => (
        <div
          key={tx.id}
          className="bg-[#0a0d12] border border-[#1a1f28] rounded-2xl p-4 flex items-center justify-between gap-3 hover:border-gray-800 transition-all"
        >
          {/* Left: Dynamic Token Shape representation matching screenshot data layout */}
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="text-left min-w-0">
              ...
              <div className="text-[11px] text-gray-500 font-mono mt-1 flex items-center gap-1.5 min-w-0">
          ...
          {/* Right: Finished status Pill wrapper context */}
          <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
            <span className={`text-[11px] font-semibold px-3 py-1 rounded-full flex items-center gap-1 ${
              ...
              tx.status === "Pending" ? "bg-[#1a1f28] text-amber-400" : "bg-[#2a1414] text-red-400"
            }`}>
              {tx.status === "Approved" ? (
                <>
